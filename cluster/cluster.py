...
class Cluster(object):
    ...
    def __init__(self, name):
        ...
        p0, p1 = pipe(self.ctx)
        self.agent = InterfaceAgent(self.ctx, p1, name)
...
class InterfaceAgent(object):
    ...
    def __init__(self, ctx, pipe, name, loop=None):
        self.ctx = ctx
        self.pipe = pipe
        if loop is None:
            loop = IOLoop.instance()
        self.loop = loop
        self.udp = udplib.UDP(PING_PORT_NUMBER, name)
    ...
    def handle_beacon(self, fd, event):
        uuid, name, address, port = self.udp.recv(BYTES)
        if name != self.name:
            return
        ...
        if uuid in self.peers:
            ...
            self.peers[uuid] = Peer(uuid, address, port)
            self.pipe.send_multipart([b'JOINED', uuid, address, str(port)])
    
    def reap_peers(self):
        now = time.time()
        for peer in list(self.peers.values()):
            if peer.expires_at < now:
#                print("reaping %s" % peer.uuid, peer.expires_at, now, peer.address, peer.port)
                peer = self.peers.pop(peer.uuid)
                self.pipe.send_multipart([b'LEFT', peer.uuid, peer.address, str(peer.port)])
