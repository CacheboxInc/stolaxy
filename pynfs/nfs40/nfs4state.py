    ...
    def get_attributes(self, attrlist=None):
        ...
        for attr in attrlist:
            if attr == FATTR4_TYPE:
                ...
                self.fattr4_type = ret_dict[attr]
            elif attr == FATTR4_CHANGE:
                        ret_dict[attr] = 0#nfstime4(stat_struct.st_ctime, 0);
                    ...
                    ret_dict[attr] = "nobody@nowhere"#stat_struct.st_uid
            elif attr == FATTR4_OWNER_GROUP:
                    ret_dict[attr] = "nobody@nowhere"#stat_struct.st_gid
            elif attr == FATTR4_RAWDEV:
                    ret_dict[attr] = specdata4(0, 0) 
            elif attr == FATTR4_TIME_ACCESS:
                    ret_dict[attr] = converttime(now)#nfstime4(stat_struct.st_atime, 0);
            elif attr == FATTR4_TIME_MODIFY:
                    ret_dict[attr] = converttime(now)#nfstime4(stat_struct.st_mtime, 0);
            elif attr == FATTR4_TIME_METADATA:
                    ret_dict[attr] = converttime(now)#nfstime4(stat_struct.st_mtime, 0);
            elif attr == FATTR4_SUPPORTED_ATTRS:
                ret_dict[attr] = nfs4lib.list2bitmap([x for x in self.supported.keys() if not 'n' in self.supported[x]])
            elif attr == FATTR4_RDATTR_ERROR:
                ret_dict[attr] = NFS4_OK
